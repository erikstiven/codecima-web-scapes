...
const PlanDetails = () => {
  ...
  return (
    <Layout>
      <Seo
        title={`${plan?.name || "Plan"} | CodeCima`}
        description={`Conoce todos los detalles del ${plan?.name}. Descubre sus características, galería y cómo contratarlo.`}
        keywords={`${plan?.name}, planes web Ecuador, servicios digitales, desarrollo web`}
        image={plan?.gallery?.[0]?.url || "/images/preview.webp"}
        url={`https://www.codecima.com.ec/precios/${categoryId}/${planId}`}
      />
            ...
            <div className="md:col-span-2 space-y-8">
              <AnimatedCard>
                ...
                <div className="text-gray-300 prose prose-invert prose-blue max-w-none space-y-4">
                  {(plan.fullDescription || []).map((line, index) => (
                ...
                <GlowButton
                  ...
                  onClick={() => {
                    const mensaje = encodeURIComponent(
                      `Hola, estoy interesado en el plan *${plan.name}* (${plan.categoryTitle}).\n\n` +
                      `¿Podrían darme más detalles y una cotización personalizada?\n\nGracias.`
                    );
                    window.open(
                      `https://wa.me/593999406026?text=${mensaje}`,
                      "_blank"
                    );
                  }}
                >
