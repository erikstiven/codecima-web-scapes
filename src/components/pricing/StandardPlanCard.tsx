...
const StandardPlanCard: React.FC<StandardPlanCardProps> = ({
  plan,
  onClick,
}) => {
  return (
    <div className="relative h-full flex flex-col">
      ...
      >
        ...
        >
          ...
          <div className="text-white/90 mb-4 min-h-[50px]">
            {plan.description}
          </div>
      ...
      ${
        plan.highlighted
          ? "bg-gradient-to-r from-orange-500 to-yellow-400 hover:brightness-110 shadow-md"
          : "bg-white/10 hover:bg-codecima-blue/60 border border-white/10"
      }`}
            >
              {plan.highlighted ? "¡Lo quiero! - 30% DCTO" : "¡Lo quiero!"}
            </button>
          </div>
        </div>
      </div>
